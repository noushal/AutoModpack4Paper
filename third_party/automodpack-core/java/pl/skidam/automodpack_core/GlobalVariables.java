...
public class GlobalVariables {
    ...
    // AutoModpack4Paper modification: paths above are non-final and relocated into the plugin data folder.
    // Must be called before any other core class that captures these paths is loaded (e.g. SecretsStore).
    public static void relocate(Path automodpackRoot, Path contentDir) {
        automodpackDir = automodpackRoot;
        hostModpackDir = automodpackDir.resolve("host-modpack");
        hostContentModpackDir = contentDir;
        ...
        privateDir = automodpackDir.resolve(".private");
        serverSecretsFile = privateDir.resolve("automodpack-secrets.json");
        knownHostsFile = privateDir.resolve("automodpack-known-hosts.json");
        serverCertFile = privateDir.resolve("cert.crt");
        serverPrivateKeyFile = privateDir.resolve("key.pem");
    }
}
